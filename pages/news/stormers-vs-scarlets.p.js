import {Template} from "../_template";
import styles from "../../styles/Projects.module.css";
import Head from "next/head";
import { useMediaQuery } from './../useMediaQuery';
import Image from "next/image";

export default function Page() {
    const md = useMediaQuery(960);
    return <><Head>
        <title>Stormers vs Scarlets</title>
        <meta name="keywords" content="news,trending,latest,sports,breaking,top,Stormers vs Scarlets"/>
        <meta name="description" content="Trending News about Stormers vs Scarlets" /></Head><Template>
            <h1 style={{fontSize: "30"}}>Stormers vs Scarlets</h1>
            <Image width={800} height={500} src="https://cdn.24.co.za/files/Cms/General/d/7788/f2b4bb8cdf3343d8a6997b6cde0eaa99.jpg" alt="Stormers vs Scarlets"/>
            <h3>Recent News</h3>
            <a href='https://www.news24.com/sport/rugby/unitedrugbychampionship/defending-champion-stormers-show-off-depth-carve-scarlets-apart-in-first-half-blitz-20221125'>
            <div className={styles.card} style={{ maxWidth: "90%", width: md ? "auto": "90vh"}}>
                <h3>Defending champion Stormers show off depth, carve Scarlets apart ...</h3>
                <div style={{ display: "flex" }}>
                <Image src="https://t1.gstatic.com/images?q=tbn:ANd9GcTQDUw-rIzdx4cpvnlxh7RerJloB685aU-PZSIpwMDmjAyaxuh_ta9jNsfnwKZ-eupujtp-Ab7R" alt="Defending champion Stormers show off depth, carve Scarlets apart ..." height={150} width={150} style={{marginLeft: "15px"}}/>
                <p style={{marginLeft: "15px"}}>The Stormers flexed their muscles, showing the depth they have in trouncing Wales&#39; Scarlets at Cape Town Stadium in a one-side United Rugby Championship&nbsp;...</p></div>
            </div>
        </a><a href='https://www.iol.co.za/sport/rugby/urc/can-kade-wolhuter-show-manie-traits-for-stormers-against-scarlets-6d868c24-3ceb-4803-b714-841980626922'>
            <div className={styles.card} style={{ maxWidth: "90%", width: md ? "auto": "90vh"}}>
                <h3>Can Kade Wolhuter show &#39;Manie traits&#39; for Stormers against Scarlets?</h3>
                <div style={{ display: "flex" }}>
                <Image src="https://t3.gstatic.com/images?q=tbn:ANd9GcSD7BCCWqrSRVjuMtMLUyQ0RpkICIhGW56quixoMXMN5v_L_8hd2xOzMr5Kb2gRm0JkDwtq8WOj" alt="Can Kade Wolhuter show &#39;Manie traits&#39; for Stormers against Scarlets?" height={150} width={150} style={{marginLeft: "15px"}}/>
                <p style={{marginLeft: "15px"}}>Stormers coach John Dobson remarked on Thursday that the Llanelli outfit had nearly 300 Test caps in their match-23 compared to about 24 in his squad,&nbsp;...</p></div>
            </div>
        </a><a href='https://www.thestatszone.com/rugby-union/united-rugby-championship/stormers-vs-scarlets-preview-prediction-2022-23-united-rugby-championship'>
            <div className={styles.card} style={{ maxWidth: "90%", width: md ? "auto": "90vh"}}>
                <h3>Stormers vs Scarlets Preview &amp; Prediction | 2022-23 United Rugby ...</h3>
                <div style={{ display: "flex" }}>
                <Image src="https://t1.gstatic.com/images?q=tbn:ANd9GcRQUxmCknhpmmi6Mev3uGSxXFIohiTSYMiFRzdblbgRVmX6BxOxZAzudCtr2DmK7cJlmfNwcN2n" alt="Stormers vs Scarlets Preview &amp; Prediction | 2022-23 United Rugby ..." height={150} width={150} style={{marginLeft: "15px"}}/>
                <p style={{marginLeft: "15px"}}>Stormers vs Scarlets match preview: TV &amp; streaming details, ticket information, free betting tips &amp; prediction…</p></div>
            </div>
        </a><a href='https://www.rugbypass.com/news/scarlets-beaten-again-after-stormers-dominate-first-half-scoring-in-cape-town/'>
            <div className={styles.card} style={{ maxWidth: "90%", width: md ? "auto": "90vh"}}>
                <h3>Scarlets beaten again after Stormers dominate first-half scoring in ...</h3>
                <div style={{ display: "flex" }}>
                <Image src="https://t3.gstatic.com/images?q=tbn:ANd9GcRSbc8qHKzY6L2dUYYeq2gukc2MbJVHKgAvUg5Xr5t9jSho_b3QswJYnutawr8lLjr4oeiVPXtM" alt="Scarlets beaten again after Stormers dominate first-half scoring in ..." height={150} width={150} style={{marginLeft: "15px"}}/>
                <p style={{marginLeft: "15px"}}>The Scarlets&#39; first-half failings cost them dear as they slipped to a 36-19 defeat against a weakened Stormers side.</p></div>
            </div>
        </a><a href='https://www.sarugbymag.co.za/stormers-punish-sleepy-scarlets/'>
            <div className={styles.card} style={{ maxWidth: "90%", width: md ? "auto": "90vh"}}>
                <h3>Stormers punish sleepy Scarlets</h3>
                <div style={{ display: "flex" }}>
                <Image src="https://t2.gstatic.com/images?q=tbn:ANd9GcQF_F8kLzjthDa8fhq6YH-KIWzP-37l2UhJbzFeUPb1t9H6dXfZUo7sYxuz7Gh7TCy70ayDru4I" alt="Stormers punish sleepy Scarlets" height={150} width={150} style={{marginLeft: "15px"}}/>
                <p style={{marginLeft: "15px"}}>The DHL Stormers marked the return of the Vodacom United Rugby Championship with a sensational performance earning the hosts a bonus-point victory over&nbsp;...</p></div>
            </div>
        </a><a href='https://www.thesouthafrican.com/sport/rugby/united-rugby-championship/urc-live-scores-stormers-vs-scarlets-result-breaking-25-november-2022/'>
            <div className={styles.card} style={{ maxWidth: "90%", width: md ? "auto": "90vh"}}>
                <h3>Stormers thump the Scarlets in Cape Town romp</h3>
                <div style={{ display: "flex" }}>
                <Image src="https://t3.gstatic.com/images?q=tbn:ANd9GcTrxLrUULEilEHaqeEmkB_tDGbGRLUTIWwn5EUN2FoBkgSX_Rf2imPQ6-uz_p57SnoS_3t_m_VT" alt="Stormers thump the Scarlets in Cape Town romp" height={150} width={150} style={{marginLeft: "15px"}}/>
                <p style={{marginLeft: "15px"}}>Follow the Stormers vs Scarlets with live score updates from the United Rugby Championship on Friday 25 November 2022.</p></div>
            </div>
        </a><a href='https://www.bbc.co.uk/sport/rugby-union/63760871'>
            <div className={styles.card} style={{ maxWidth: "90%", width: md ? "auto": "90vh"}}>
                <h3>Scarlets slump to convincing defeat by Stormers</h3>
                <div style={{ display: "flex" }}>
                <Image src="https://t1.gstatic.com/images?q=tbn:ANd9GcQjgd6KVXupRewBpjgF4tquFIGN26iFOo4MTJ_Cy60COnt5H__lyFzJq-h1s4WdxXPU3jnDUTcC" alt="Scarlets slump to convincing defeat by Stormers" height={150} width={150} style={{marginLeft: "15px"}}/>
                <p style={{marginLeft: "15px"}}>Ryan Conbeer scores twice but Scarlets leave empty handed from Cape Town against the defending URC champions.</p></div>
            </div>
        </a>
        </Template></>;
}